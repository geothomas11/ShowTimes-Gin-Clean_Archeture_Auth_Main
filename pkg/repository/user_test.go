	...
	tests := []struct {
	...
	}{
		{
			...
			stub: func(mockSQL sqlmock.Sqlmock) {
				expectedQuery := `^SELECT u.id, u.name, u.email, u.phone, r.referral_code FROM users u INNER JOIN referrals r ON u.id = r.user_id WHERE u.id = \$1`
				mockSQL.ExpectQuery(expectedQuery).WithArgs(1).
					WillReturnRows(sqlmock.NewRows([]string{"id", "name", "email", "phone", "referral_code"}).
						AddRow(1, "Arun", "arun@gmail.com", "9746359523", "christmas200"))
			},
			want: models.UsersProfileDetails{
				ID:           1,
				Name:         "Arun",
